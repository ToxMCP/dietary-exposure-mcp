...
def test_defaults_manifest_contains_versioned_files() -> None:
    ...
    assert any(
        item["path"].endswith("food_vocabulary_crosswalk.json") for item in manifest["files"]
    )
    assert any(item["path"].endswith("model_governance.json") for item in manifest["files"])
    assert any(
        item["path"].endswith("regulatory_readiness_profiles.json") for item in manifest["files"]
    )
    assert any(item["path"].endswith("reference_values.json") for item in manifest["files"])
    assert any(
        item["path"].endswith("contaminant_legal_limits_wave2.json") for item in manifest["files"]
    )
    ...
    assert any(
        item["path"].endswith("jurisdiction_coverage_wave1.json") for item in manifest["files"]
    )
    assert any(item["path"].endswith("reporting_profiles.json") for item in manifest["files"])
    assert any(
        item["path"].endswith("occurrence_evidence_registry.json") for item in manifest["files"]
    )
    assert any(
        item["path"].endswith("analytical_method_evidence_registry.json")
        for item in manifest["files"]
    )
    assert any(
        item["path"].endswith("metals_occurrence_registry.json") for item in manifest["files"]
    )
    assert any(
        item["path"].endswith("metals_review_focus_registry.json") for item in manifest["files"]
    )
...
def test_defaults_infer_source_origin_tags_and_reference_provenance() -> None:
    ...
    assert registry.source_catalog_reference(
        "dietary.profile.eu_adult_general"
    ).origin_tag.value == ("illustrative_placeholder")
...
def test_screening_defaults_cover_broader_populations_and_commodities() -> None:
    ...
    population_groups = {
        item["populationGroup"] for item in registry.consumption_profiles_manifest()["profiles"]
    }
    commodity_codes = {
        item["commodityCode"] for item in registry.commodity_taxonomy_manifest()["commodities"]
    }
    food_vocab_codes = {
        item["commodityCode"]
        for item in registry.food_vocabulary_crosswalk_manifest()["commodityMappings"]
    }
...
def test_reference_value_conflicts_and_emerging_contaminant_registries_are_available() -> None:
    ...
    glyphosate_records = [
        ...
        if item["substanceKey"] == "glyphosate"
    ]
    acetamiprid_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "acetamiprid"
    ]
    ethiprole_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "ethiprole"
    ]
    tetraconazole_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "tetraconazole"
    ]
    tebuconazole_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "tebuconazole"
    ]
    glufosinate_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "glufosinate"
    ]
    oxamyl_records = [
    ...
    difenoconazole_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "difenoconazole"
    ]
    pfas_record = next(
        item
        for item in registry.list_reference_value_records()
    ...
    acrylamide_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "acrylamide"
    ]
    bpa_record = next(
        item
        for item in registry.list_reference_value_records()
    ...
    lead_records = [
        item for item in registry.list_reference_value_records() if item["substanceKey"] == "lead"
    ]
    ...
    cadmium_occurrence = registry.get_metals_occurrence_record(
        "efsa.cadmium.occurrence_monitoring.support"
    )
    lead_occurrence = registry.get_metals_occurrence_record(
        "efsa.lead.occurrence_monitoring.support"
    )
    inorganic_arsenic_occurrence = registry.get_metals_occurrence_record(
        "efsa.inorganic_arsenic.occurrence_monitoring.support"
    )
    mercury_occurrence = registry.get_metals_occurrence_record(
        "efsa.mercury.occurrence_monitoring.support"
    )
    mercury_review_focus = registry.get_metals_review_focus_record(
        "efsa.mercury.large_predatory_fish.review_focus"
    )
    arsenic_review_focus = registry.get_metals_review_focus_record(
    ...
    imidacloprid_records = [
        item
        for item in registry.list_reference_value_records()
        if item["substanceKey"] == "imidacloprid"
    ]
    ...
    nl_pfas_reporting_profile = registry.get_reporting_profile_record(
        "nl.pfas.rivm_peq.food_advisory"
    )
    nl_pfas_biota_reporting_profile = registry.get_reporting_profile_record(
        "nl.pfas.rivm_peq.biota_fish_advisory"
    )
    pfas_reporting_profiles = registry.get_reporting_profile_records_for_family(
        "pfas_food_contaminants"
    )
    ...
    inorganic_arsenic = registry.get_emerging_contaminant_record(
    ...
    assert {item["recordId"] for item in imidacloprid_records} == {
        ...
        "efsa.openfoodtox.imidacloprid.arfd.2008",
        "jmpr.imidacloprid.adi.2001",
        "cn.nhc.imidacloprid.adi.2026",
    }
    imidacloprid_arfd = {
        item["recordId"]: item for item in imidacloprid_records if item["referenceType"] == "arfd"
    }
    assert {item["unit"] for item in imidacloprid_arfd.values()} == {"mg/kg bw"}
    assert {item["documentStatus"] for item in imidacloprid_arfd.values()} == {
        "dataset_current",
        "final_current",
    }
    assert {item["conflictGroupId"] for item in imidacloprid_arfd.values()} == {
        "imidacloprid.arfd.efsa_context"
    }
    ...
    assert cadmium_occurrence["datasetIds"] == [
    ...
    assert inorganic_arsenic_occurrence["matrixScope"] == [
        "broad_food_supply",
        "rice_and_cereal_products",
    ]
    ...
    assert (
        ...
        in mercury_occurrence["sensitivePopulationGroups"]
    )
    assert mercury_review_focus["linkedOccurrenceRecordIds"] == [
        "efsa.mercury.occurrence_monitoring.support"
    ]
    ...
    assert (
        "efsa.mercury.large_predatory_fish.review_focus"
        in mercury_occurrence_evidence["linkedReviewFocusIds"]
    )
    ...
    assert acrylamide_coffee_occurrence_evidence["matrixGroups"] == [
        "coffee_and_coffee_substitutes"
    ]
    ...
    assert (
        "efsa.pesticide_residues.organic_findings.2025"
        in imidacloprid_occurrence_evidence["sourceIds"]
    )
    ...
    assert (
        "efsa.difenoconazole.import_tolerance.2025"
        in difenoconazole_occurrence_evidence["sourceIds"]
    )
    ...
    assert (
        "efsa.pesticide_residues.organic_findings.2025" in imidacloprid_method_evidence["sourceIds"]
    )
...
def test_wave1_jurisdiction_expansion_records_are_available() -> None:
    ...
    us_authority = registry.get_legal_authority_record(
        "us.epa.pesticides.40cfr_part_180.govinfo_2024"
    )
    codex_authority = registry.get_legal_authority_record(
        "codex.cccf.cxs_193_1995.current_2025.cadmium"
    )
...
def test_wave1_jurisdiction_coverage_registry_is_available() -> None:
    ...
    us_glyphosate = registry.get_jurisdiction_coverage_record(
        "us.pesticide_residue.glyphosate.wave1"
    )
    codex_cadmium = registry.get_jurisdiction_coverage_record(
        "codex_global.cadmium_food_contaminants.cadmium.wave1"
    )
    china_lead = registry.get_jurisdiction_coverage_record("cn.lead_food_contaminants.lead.wave1")
    us_lead_limit = registry.get_contaminant_legal_limit_record(
        "us.fda.lead.processed_foods.general_baby_foods.ml.2025"
    )
    china_arsenic_limit = registry.get_contaminant_legal_limit_record(
    ...
    assert china_lead["officialSourceIds"] == [
        "cn.nhc.gb2762.2025.announcement",
        "cn.cfsa.gb2762.2025.standard_text",
    ]
...
def test_duplicate_model_governance_entries_fail(tmp_path: Path) -> None:
    ...
    (tmp_path / "defaults" / "v1" / "model_governance.json").write_text(
...
def test_unknown_reference_value_source_fails(tmp_path: Path) -> None:
    ...
    (tmp_path / "defaults" / "v1" / "reference_values.json").write_text(
        json.dumps(payload, indent=2) + "\n"
    )
...
def test_wave1_source_pack_rejects_non_official_origin(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "source_catalog_jurisdiction_wave1.json").read_text()
    )
...
def test_wave1_reference_value_pack_requires_effective_date(
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "reference_values_jurisdiction_wave1.json").read_text()
    )
...
def test_wave1_legal_authority_pack_requires_official_source(
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "legal_authorities_jurisdiction_wave1.json").read_text()
    )
...
def test_wave1_mrl_pack_requires_effective_date(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "mrl_enforcement_jurisdiction_wave1.json").read_text()
    )
    payload["records"][0].pop("effectiveDate", None)
...
def test_wave1_mrl_pack_requires_official_source(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
...
def test_jurisdiction_coverage_pack_rejects_unknown_legal_authority(
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "jurisdiction_coverage_wave1.json").read_text()
    )
...
def test_wave2_contaminant_legal_limit_pack_requires_effective_date(
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "contaminant_legal_limits_wave2.json").read_text()
    )
    payload["records"][0].pop("effectiveDate", None)
    ...
    with pytest.raises(DietaryRegistryError) as exc_info:
        DefaultsRegistry(tmp_path)
    assert (
        exc_info.value.payload.code
        == "missing_official_primary_contaminant_legal_limit_effective_date"
    )


def test_duplicate_active_wave1_mrl_records_fail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
...
def test_unknown_metals_occurrence_method_fails(tmp_path: Path) -> None:
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "metals_occurrence_registry.json").read_text()
    )
...
def test_unknown_metals_review_focus_occurrence_record_fails(tmp_path: Path) -> None:
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "metals_review_focus_registry.json").read_text()
    )
...
def test_unknown_occurrence_evidence_review_focus_fails(tmp_path: Path) -> None:
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "occurrence_evidence_registry.json").read_text()
    )
...
def test_unknown_analytical_method_evidence_method_fails(tmp_path: Path) -> None:
    ...
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
    payload = json.loads(
        (tmp_path / "defaults" / "v1" / "analytical_method_evidence_registry.json").read_text()
    )
...
def test_manifest_verification_can_be_skipped_via_env_var(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    shutil.copytree(repo_root / "defaults", tmp_path / "defaults")
